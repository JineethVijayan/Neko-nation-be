...
export const deleteAddress = async (req, res) => {
  try {
    ...
    const order = await Order.findOne({ address: id });
    ...
    if (order.status === 'canceled' || order.status === 'delivered') {
      ...
      await DeliveryAddress.deleteOne({ _id: id })
      return res.status(200).json({ message: 'Address deleted successfully' });
      
    } else {
      return res.status(400).json({ message: "Cannot delete address, it is used in an order" });
    }
